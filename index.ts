class SimplicialComplex {
    ...
    public merge(other: SimplicialComplex) {
...
class PersistentHomology {
    ...
    private getComplexContainingPoint(p: number) {
        const complex = this.complexes.find((complex) => {
    ...
    private mergeComplexes(a: SimplicialComplex, b: SimplicialComplex) {
        // Older complex has more points
        if (a.size() >= b.size()) {
            // Merge "b" into "a"
            a.merge(b);
            // Remove "a" from complexes (complex death)
            this.complexes.splice(this.complexes.indexOf(b), 1);
        } else {
            // Merge "a" into "b"
            b.merge(a);
            // Remove "b" from complexes (complex death)
            this.complexes.splice(this.complexes.indexOf(a), 1);
        }
    }
    ...
    public execute() {
        ...
        for (let threshold = this.min; threshold < this.max + this.resolution; threshold += this.resolution) {
            ...
            if (ranges.length > 0) {
                ...
                ranges.forEach((range) => {
                    ...
                    if (left <= value && right <= value) {
                        ...
                        this.mergeComplexes(leftComplex, rightComplex);
